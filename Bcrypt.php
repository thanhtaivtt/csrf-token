...
class Bcrypt {
     ...
     * @var array
     */
    private static $validIdentifiers = array ('2a', '2x', '2y');
    ...
    private static function _validateIdentifier($hash) {
        if (!in_array(substr($hash, 1, 2), self::$validIdentifiers)) {
            throw new Exception('Unsupported hash format.');
        }
    }
